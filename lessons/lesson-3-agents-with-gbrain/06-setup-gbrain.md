...
Самая инфраструктурная часть урока. Нужно завести Second Brain – Postgres+pgvector с четырьмя MCP-серверами поверх. Хорошие новости: для этого есть готовый дистрибутив `public-gbrain-agentos`, который сам себя устанавливает через Claude Code.
...
6. Создаём три персональных Bearer-токена (для coder / marketer / sales)
...
| Agent identities | `homer,kael,closer` (через запятую) |
| Init vault | `yes` |
...
```text
Создай три персональных Bearer-токена в agent_tokens:
- homer (scopes: 20-daily, 30-decisions, 70-runbooks, 80-error-patterns, 90-inbox, 50-external)
- kael (те же scopes)
- closer (те же scopes)
...
echo "<kael-bearer-token>"    > ~/.claude-lab/kael/secrets/gbrain.token
echo "<closer-bearer-token>" > ~/.claude-lab/closer/secrets/gbrain.token

chmod 600 ~/.claude-lab/{homer,kael,closer}/secrets/gbrain.token
```
