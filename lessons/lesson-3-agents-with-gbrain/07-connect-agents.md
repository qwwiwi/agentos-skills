...
1. В каждый workspace (`homer`, `kael`, `closer`) кладём `.mcp.json` с четырьмя MCP-серверами gbrain
...
## Шаг 1: .mcp.json для coder (Хомер)
...
## Шаг 2: .mcp.json для marketer (Маркет)

```bash
TOKEN=$(cat ~/.claude-lab/kael/secrets/gbrain.token)
GBRAIN_URL="https://your-gbrain.example.com"

cat > ~/.claude-lab/kael/.claude/.mcp.json <<EOF
{
  "mcpServers": {
    "gbrain-memory": {
      "type": "http",
...
chmod 600 ~/.claude-lab/kael/.claude/.mcp.json
```

## Шаг 3: .mcp.json для sales (Клозер)

```bash
TOKEN=$(cat ~/.claude-lab/closer/secrets/gbrain.token)
GBRAIN_URL="https://your-gbrain.example.com"

cat > ~/.claude-lab/closer/.claude/.mcp.json <<EOF
{
  "mcpServers": {
    "gbrain-memory": {
      "type": "http",
...
chmod 600 ~/.claude-lab/closer/.claude/.mcp.json
```

## Шаг 4: проверяем синтаксис

```bash
for agent in homer kael closer; do
  echo "=== $agent ==="
...
Где `<my_name>` – `homer` / `kael` / `closer` соответственно. В шаблонах из `agentos-skills/templates/claude-md/` эта секция уже есть, плейсхолдер `<my_name>` заменяется на этапе sed (см. шаги 03-05).

## Шаг 6: smoke-тест recall

Запусти Claude Code в одном из workspace, например, в Хомере:
...
Пусто – нормально, никто Хомеру ничего не слал. Главное – 200 OK.
...
cd ~/.claude-lab/kael/.claude && claude    # затем Heartbeat-вызов
cd ~/.claude-lab/closer/.claude && claude # затем Heartbeat-вызов
```
...
Ожидаемо: три записи, у всех `status: online`, `last_seen` – недавно.
...
Симптом: heartbeat от Маркета в `agent_list` показан как `homer`. Значит у тебя один и тот же токен в .mcp.json у разных агентов. Перепроверь – каждый workspace должен иметь свой токен из своего `secrets/gbrain.token`.
